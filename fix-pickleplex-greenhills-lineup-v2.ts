...
async function main() {
  ...
  const chris = stopTeamPlayers.find(stp => 
    stp.player.name === 'Chris Kostrzewa' || 
    (stp.player.firstName === 'Chris' && stp.player.lastName === 'Kostrzewa')
  );
  const michael = stopTeamPlayers.find(stp => 
    stp.player.name === 'Michael Hampson' || 
    (stp.player.firstName === 'Michael' && stp.player.lastName === 'Hampson')
  );
  const stefi = stopTeamPlayers.find(stp => 
    stp.player.name === 'Stefanie Schaefler' || 
    (stp.player.firstName === 'Stefanie' && stp.player.lastName === 'Schaefler')
  );
  const ali = stopTeamPlayers.find(stp => 
    stp.player.name === 'Ali Underhill' || 
    (stp.player.firstName === 'Ali' && stp.player.lastName === 'Underhill')
  );
  ...
  console.log('\nFinal lineups:');
  console.log('MENS_DOUBLES: Chris Kostrzewa & Michael Hampson vs Gene Liang & Josh Hazenbuhler');
  console.log('WOMENS_DOUBLES: Stefanie Schaefler & Ali Underhill vs Diana Hatch & Trina Ngu');
  console.log('MIXED_1: Michael Hampson & Stefanie Schaefler vs Diana Hatch & Josh Hazenbuhler');
  console.log('MIXED_2: Chris Kostrzewa & Ali Underhill vs Gene Liang & Trina Ngu');
}
