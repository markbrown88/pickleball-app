...
async function main() {
  ...
  const matchId = 'cmgdy7yx400c3r0k8zyye5c1y'; // Real Pickleball Advanced vs Blue Zone Advanced
  ...
  const jamie = stopTeamPlayers.find(stp => 
    stp.player.firstName === 'Jamie' || stp.player.name?.includes('Jamie')
  );
  const tom = stopTeamPlayers.find(stp => 
    stp.player.firstName === 'Tom' || stp.player.name?.includes('Tom')
  );
  const reese = stopTeamPlayers.find(stp => 
    stp.player.firstName === 'Reese' || stp.player.name?.includes('Reese')
  );
  const steffi = stopTeamPlayers.find(stp => 
    stp.player.firstName === 'Steffi' || stp.player.name?.includes('Steffi')
  );
  ...
  console.log('\nFinal lineups:');
  console.log('MENS_DOUBLES: Ryan Bilodeau & Troy Rieck vs Jamie ? & Tom ?');
  console.log('WOMENS_DOUBLES: Leanna Macdonnell & Jojo Phang vs Reese ? & Steffi ?');
  console.log('MIXED_1: Leanna Macdonnell & Troy Rieck vs Jamie ? & Reese ?');
  console.log('MIXED_2: Ryan Bilodeau & Jojo Phang vs Tom ? & Steffi ?');
}
