...
export default function MePage() {
  ...
  const startGame = async (gameId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const matchId = Object.keys(games).find(matchId => 
          games[matchId].some(game => game.id === gameId)
        );
        if (matchId) {
          setMatchStatuses(prev => ({ ...prev, [matchId]: 'in_progress' }));
        }
  ...
  const endGame = async (gameId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to end game:', response.status);
      }
    } catch (error) {
  ...
  const loadGamesForMatch = async (matchId: string) => {
    // Prevent duplicate calls for the same match
    if (games[matchId]) {
      console.log('Games already loaded for match:', matchId);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const statusUpdates: Record<string, 'not_started' | 'in_progress' | 'completed'> = {};
        gamesData.forEach((game: any) => {
          if (game.status) {
            statusUpdates[game.id] = game.status;
          }
        });
        if (Object.keys(statusUpdates).length > 0) {
          setGameStatuses(prev => ({ ...prev, ...statusUpdates }));
        }
      } else {
  ...
  const completeGame = async (gameId: string) => {
    try {
      ...
      // Update both isComplete and status in database
      const response = await fetch(`/api/admin/games/${gameId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isComplete: true, status: 'completed' })
      });
...
function EventManagerTab({
  ...
  const [activeStop, setActiveStop] = useState<Record<string, string>>({}); // tournamentId -> stopId
  const [activeRound, setActiveRound] = useState<Record<string, string>>({}); // stopId -> roundId
    ...
    setExpandedTournaments(prev => {
      ...
      if (newSet.has(tournamentId)) {
        ...
        if (tournament) {
          ...
          // Set first stop as active when expanding tournament
          if (tournament.stops.length > 0) {
            setActiveStop(prev => ({ ...prev, [tournamentId]: tournament.stops[0].stopId }));
          }
        }
      }
      return newSet;
  ...
  const setActiveStopForTournament = (tournamentId: string, stopId: string) => {
    setActiveStop(prev => ({ ...prev, [tournamentId]: stopId }));
    // Set first round as active when switching stops
    const stopSchedule = scheduleData[stopId];
    if (stopSchedule && stopSchedule.length > 0) {
      setActiveRound(prev => ({ ...prev, [stopId]: stopSchedule[0].id }));
    }
  };

  const setActiveRoundForStop = (stopId: string, roundId: string) => {
    setActiveRound(prev => ({ ...prev, [stopId]: roundId }));
  };
  ...
  const loadSchedule = async (stopId: string, force = false) => {
    ...
    try {
      ...
      // Set first round as active when schedule loads
      if (data && data.length > 0) {
        setActiveRound(prev => ({ ...prev, [stopId]: data[0].id }));
      }
              ...
              <div className="border-t bg-gray-50">
                <div className="flex">
                    ...
                    <div className="p-2">
                      ...
                      <div className="space-y-1">
                        {tournament.stops.map((stop) => {
                          const isActive = activeStop[tournament.tournamentId] === stop.stopId;
                          return (
                            <button
                              key={stop.stopId}
                              ...
                              onClick={() => setActiveStopForTournament(tournament.tournamentId, stop.stopId)}
                            >
                              ...
                              <div className="text-xs text-gray-400 mt-1">
                                {scheduleData[stop.stopId]?.length || 0} rounds • {scheduleData[stop.stopId]?.reduce((acc: number, r: any) => acc + (r.matches?.length || 0), 0) || 0} matches
                              </div>
                            </button>
                          );
                        })}
                  ...
                  <div className="flex-1">
                    {(() => {
                      const activeStopId = activeStop[tournament.tournamentId];
                      const stop = tournament.stops.find(s => s.stopId === activeStopId);
                      
                      if (!stop) {
                        return (
                          <div className="p-4 text-center text-gray-500">
                            Select a stop to view its schedule
                          </div>
                        );
                      }
                      ...
                      return (
                        <div className="p-4">
                          <div className="flex items-center justify-between mb-4">
                            <div>
                              <h4 className="text-lg font-semibold">{stop.stopName}</h4>
                              <p className="text-sm text-gray-600">
                                {stop.locationName && `${stop.locationName} • `}
                                {formatDate(stop.startAt ?? null)} - {formatDate(stop.endAt ?? null)}
                              </p>
                          ...
                          {loading[stop.stopId] ? (
                            ...
                            <div>
                              {/* Round Tabs - Horizontal */}
                              <div className="border-b border-gray-200 mb-4">
                                <nav className="flex space-x-8">
                                  {scheduleData[stop.stopId].map((round, roundIdx) => {
                                    const isActive = activeRound[stop.stopId] === round.id;
                                    return (
                                      <button
                                        key={round.id}
                                        ...
                                        onClick={() => setActiveRoundForStop(stop.stopId, round.id)}
                                      >
                                        Round {round.idx + 1}
                                      </button>
                                    );
                              ...
                              {/* Round Content */}
                              {(() => {
                                const activeRoundId = activeRound[stop.stopId];
                                const round = scheduleData[stop.stopId]?.find(r => r.id === activeRoundId);
                                
                                if (!round) {
                                  return (
                                    <div className="text-center py-4 text-gray-500">
                                      Select a round to view its matches
                                    </div>
                                  );
                                }

                                const matches = getMatchesForRound(round, editingRounds.has(round.id));
                                const _ = updateKey; // Force re-render when updateKey changes
                                
                                return (
                                  <div key={`${round.id}-${renderKey}-${updateKey}`} className="space-y-3">
                                    <div className="flex items-center justify-between mb-4">
                                      <h5 className="font-medium text-lg">Round {round.idx + 1}</h5>
                                      <div className="flex items-center gap-2">
                                        {editingRounds.has(round.id) ? (
                                          <button
                                          ...
                                          <span className="text-sm text-gray-500">Match in progress</span>
                                        )}
                                      </div>
                                    </div>
                                    ...
                                    {(() => {
                                      ...
                                      return Object.entries(matchesByBracket).map(([bracketName, bracketMatches]) => (
                                        <div key={bracketName} className="space-y-2">
                                          <h6 className="text-sm font-medium text-gray-700 border-b pb-1">
                                            {bracketName} ({bracketMatches.length} matches)
                                          </h6>
                                          <div className="space-y-1">
                                            {bracketMatches.map((match: any, localMatchIdx: number) => {
                                              ...
                                              return (
                                                <div key={match.id} className="p-2 bg-gray-50 rounded text-sm">
                                                  {/* Match content will go here - this is a placeholder for now */}
                                                  <div className="text-sm">
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        <div key={match.id} className="flex items-center justify-between p-2 bg-gray-50 rounded text-sm">
                                            ...
                                            <div className="flex items-center gap-3">
                                              ...
                                              {(() => {
                                                const hasAnyGameInProgress = games[match.id]?.some(game => 
                                                  gameStatuses[game.id] === 'in_progress'
                                                );
                                                return hasAnyGameInProgress && (
                                          ...
                                          <div className="flex items-center gap-2">
                                              {(() => {
                                                ...
                                                return !hasAnyGameStarted && matchStatuses[match.id] !== 'in_progress' && matchStatuses[match.id] !== 'completed' && (
                                                  <button
                                            ...
                                            <div className="space-y-1">
                                              {bracketMatches.map((match: any, localMatchIdx: number) => {
                                                ...
                                                return (
                                                  <div key={match.id} className="p-2 bg-gray-50 rounded text-sm">
                                                    {/* Confirmed Lineup Display with buttons - hide when any game is started */}
                                                    {!editingMatch && (() => {
                                                      const hasAnyGameStarted = games[match.id]?.some(game => 
                                                        gameStatuses[game.id] === 'in_progress' || gameStatuses[game.id] === 'completed'
                                                      );
                                                      return !hasAnyGameStarted && (
                                                                  ...
                                                                  <div className="flex items-center gap-2">
                                                                    {(() => {
                                                                      ...
                                                                      if (gameStatus === 'completed') {
                                                                        return null; // Hide Court # for completed games
                                                                      } else {
                                                                        return (
                                                                          <>
                                            ...
                                            >
                                                                        ...
                                                                        return null; // Hide button for completed games
                                                                      }
                                                                    })()}
                                          </div>
                                        </div>
         ...
         <div className={`text-sm text-right ${
           (() => {
             ...
             if (gameStatus === 'completed') {
               ...
               return teamAScore > teamBScore ? 'font-bold text-green-800' : 'font-medium text-gray-700';
             }
             return 'font-medium text-gray-700';
           })()
         }`}>
           {lineups[match.id][match.teamA?.id || 'teamA']?.filter(p => p.gender === (game.slot === 'MENS_DOUBLES' ? 'MALE' : 'FEMALE')).slice(0, 2).map(p => (
         ...
         <div className={`text-sm text-left ${
           (() => {
             ...
             if (gameStatus === 'completed') {
               const teamAScore = game.teamAScore || 0;
               const teamBScore = game.teamBScore || 0;
               return teamBScore > teamAScore ? 'font-bold text-green-800' : 'font-medium text-gray-700';
             }
             return 'font-medium text-gray-700';
           })()
         }`}>
           {lineups[match.id][match.teamB?.id || 'teamB']?.filter(p => p.gender === (game.slot === 'MENS_DOUBLES' ? 'MALE' : 'FEMALE')).slice(0, 2).map(p => (
             <div key={p.id}>{p.name}</div>
           ))}
         </div>
       </div>
                                                                  ...
                                                                  <div className={`text-sm text-right ${
                                                                    ...
                                                                    {(() => {
                                                                      ...
                                                                      if (game.slot === 'MIXED_1') {
                                                                        return (
                                                                          <>
                                                                            <div>{males[0]?.name || ''}</div>
                                                                            <div>{females[0]?.name || ''}</div>
                                                                          </>
                                                                        );
                                                                      } else if (game.slot === 'MIXED_2') {
                                                                        return (
                                                                          <>
                                                                            <div>{males[1]?.name || ''}</div>
                                                                            <div>{females[1]?.name || ''}</div>
                                                                          </>
                                                                        );
                                                                      }
                                                                      return '';
                                                          ...
                                                          {(() => {
                                                            ...
                                                            if (needsTiebreaker && !games[match.id]?.find(g => g.slot === 'TIEBREAKER') && !creatingTiebreakers.has(match.id)) {
                                                              // Create tiebreaker game in database
                                                              const createTiebreaker = async () => {
                                                                setCreatingTiebreakers(prev => new Set(prev).add(match.id));
                                                                try {
                                                            ...
                                                            return needsTiebreaker && games[match.id]?.find(g => g.slot === 'TIEBREAKER') && (
                                                              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded">
                                                                <div className="flex items-center justify-between mb-2">
                                                                  <div className="flex items-center gap-2">
                                                                    <div className="text-xs font-bold text-yellow-800">Tiebreaker</div>
                                                                    {/* Game in Progress indicator */}
                                                                    {(() => {
                                                                      const tiebreakerGame = games[match.id]?.find(g => g.slot === 'TIEBREAKER');
                                                                      return tiebreakerGame && gameStatuses[tiebreakerGame.id] === 'in_progress' && (
                                                                        <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded font-medium">
                                                                          Game in Progress
                                                                        </span>
                                                                      );
                                                                  ...
                                                                  <div className="flex items-center gap-2">
                                                                    {(() => {
                                                                      ...
                                                                      const gameStatus = gameStatuses[tiebreakerGame.id] || 'not_started';
                                                                      if (gameStatus === 'completed') {
                                                                        return null; // Hide Court # for completed games
                                                                      } else {
                                                                        return (
                                                                          <>
                                                                            ...
                                                                            <input
                                                                              ...
                                                                              value={tiebreakerGame.courtNumber || ''}
                                                                              onChange={(e) => {
                                                                    ...
                                                                    {(() => {
                                                                      ...
                                                                      if (gameStatus === 'not_started') {
                                                                        return (
                                                                          <button
                                                                            className="px-2 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700"
                                                                            onClick={() => startGame(tiebreakerGame.id)}
                                                                          >
                                                                            Start Game
                                                                          </button>
                                                                        );
                                                                        ...
                                                                        return (
                                                                          <button
                                                                            className="px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700"
                                                                            onClick={() => endGame(tiebreakerGame.id)}
                                                                          >
                                                                            End Game
                                                                          </button>
                                                                        );
                                                                ...
                                                                <div className="grid gap-2 items-center" style={{gridTemplateColumns: '1fr auto auto auto 1fr'}}>
                                                                  <div className={`text-sm text-right ${
                                                                    (() => {
                                                                      const tiebreakerGame = games[match.id]?.find(g => g.slot === 'TIEBREAKER');
                                                                      if (!tiebreakerGame) return 'font-medium text-gray-700';
                                                                      const gameStatus = gameStatuses[tiebreakerGame.id] || 'not_started';
                                                                      if (gameStatus === 'completed') {
                                                                        const teamAScore = tiebreakerGame.teamAScore || 0;
                                                                        const teamBScore = tiebreakerGame.teamBScore || 0;
                                                                        return teamAScore > teamBScore ? 'font-bold text-green-800' : 'font-medium text-gray-700';
                                                                      }
                                                                      return 'font-medium text-gray-700';
                                                                    })()
                                                                  }`}>{match.teamA?.name || 'Team A'}</div>
                                                                  {(() => {
                                                                    const tiebreakerGame = games[match.id]?.find(g => g.slot === 'TIEBREAKER');
                                                                    if (!tiebreakerGame) return null;
                                                                    const gameStatus = gameStatuses[tiebreakerGame.id] || 'not_started';
                                                                      ...
                                                                      return (
                                                                        <>
                                                                          <input
                                                                            ...
                                                                            min="0"
                                                                            className="w-12 px-1 py-0.5 text-xs border rounded text-center"
                                                                            value={tiebreakerGame.teamAScore || ''}
                                                                            onChange={(e) => {
                                                                          ...
                                                                          <input
                                                                            ...
                                                                            min="0"
                                                                            className="w-12 px-1 py-0.5 text-xs border rounded text-center"
                                                                            value={tiebreakerGame.teamBScore || ''}
                                                                            onChange={(e) => {
