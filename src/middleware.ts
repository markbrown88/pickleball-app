...
export default clerkMiddleware(async (auth, req) => {
  ...
  // Only check routes under /(player) group: /dashboard, /register, etc.
  // Exclude /profile route so users can complete their profile
  ...
  if (isPlayerRoute && !isProfileRoute(req)) {
    try {
      ...
      // If player exists but profile is incomplete (missing firstName, lastName, or clubId), redirect to profile
