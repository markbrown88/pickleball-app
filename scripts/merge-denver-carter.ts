...
async function mergeDenverCarter() {
  try {
    const keepId = 'cmiew6a9s0001jr04e4qf8pzb'; // Newer account with Clerk, email, registration (Denver Carter)
    const deleteId = 'cmgp4slqc0009jl04cykhj9sw'; // Older account with location data (Denver C)

    console.log(`\n=== Merging Denver Carter Accounts ===\n`);
    console.log(`Keep: ${keepId} (newer, has Clerk account, email, registration - Denver Carter)`);
    console.log(`Delete: ${deleteId} (older, has location data - Denver C)\n`);
    ...
    console.log(`\n✅ Successfully merged Denver Carter accounts!\n`);
    console.log(`Denver Carter (${keepPlayer.email}) now has:`);
