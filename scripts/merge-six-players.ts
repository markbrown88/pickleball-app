...
const merges: MergePair[] = [
  {
    name: 'Lucia Changkuon',
    keepId: 'cmiez14ii0001jy04oq3eisog', // Clerk + email
    deleteId: 'cmfpbp8cs002nrdn0vzqedewj', // Legacy entry with DUPR + location
  },
  {
    name: 'Shaun Hills',
    keepId: 'cmif016tb0001l504t45fj1pc', // Clerk + email
    deleteId: 'cmfpbp90s003vrdn0qxjii66u', // Legacy entry with DUPR + location
  },
  {
    name: 'Josh Aidelman',
    keepId: 'cmif1msxp0003l7042usvlaxb', // Clerk + email (Josh Aidelman)
    deleteId: 'cmfpd1j050005rdb407odr0n3', // Legacy entry (Josh A) with DUPR + location
  },
  {
    name: 'Mark Cecchetto',
    keepId: 'cmi4l40x90001l704x64cz0d5', // Clerk + email (Mark Cecchetto)
    deleteId: 'cmfpcz6r40001rdb4m9sj2mim', // Legacy entry (Mark C) with DUPR + location
  },
  {
    name: 'Jacob Fortin',
    keepId: 'cmif83elz0001i905ss59mzm3', // Clerk + email (Jacob Fortin)
    deleteId: 'cmfpbp7q9001lrdn0eso5evio', // Legacy entry (Jacob F) with DUPR + location
  },
  {
    name: 'Vanessa Volpato',
    keepId: 'cmifaj2vu0001jp04qmoh89uj', // Clerk + email (Vanessa Volpato)
    deleteId: 'cmfpffa0z000prdb4tgl86b9l', // Legacy entry (Vanessa V) with location
  },
];
...
async function mergePlayers() {
  for (const merge of merges) {
    ...
    // For players with incomplete last names, update to full last name from legacy account
    if (deletePlayer.lastName && deletePlayer.lastName.length > 1) {
      ...
      // If keep player has incomplete last name (1 char or empty) and delete has full name
