...
const merges: MergePair[] = [
  {
    name: 'Adam Ewer',
    keepId: 'cmik9nf8m0001jo045azb2iwf', // Newer account with Clerk, email
    deleteId: 'cmfpbp6su0001rdn03u347jas', // Older account with DUPR rating, location data
  },
  {
    name: 'Peter Hull',
    keepId: 'cmijbm91i0001kv04kgw8oicy', // Newer account with Clerk, email
    deleteId: 'cmfpbp8rr003drdn0ps4jyt4i', // Older account with DUPR rating, location data
  },
  {
    name: 'Eden Jiang',
    keepId: 'cmijghcfd0001jr04dfc65jcz', // Newer account with Clerk, email
    deleteId: 'cmg078cpn000zrddcciw1s69a', // Older account with location data
  },
];
...
async function mergePlayers() {
  for (const merge of merges) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // For DUPR, only merge if the delete player has a valid rating (not null and not obviously wrong like 33)
