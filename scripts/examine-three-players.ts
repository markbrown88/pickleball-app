...
async function main() {
  const player1Id = 'cmi3v3bfo0001l104ezknvykw';
  const player2Id = 'cmi3qkpm90001l804zzaz90vc';
  const player3Id = 'cmi3z8ash0001ic0444jx6ln3';

  try {
    const stats1 = await analyzePlayer(player1Id, 'PLAYER 1');
    const stats2 = await analyzePlayer(player2Id, 'PLAYER 2');
    const stats3 = await analyzePlayer(player3Id, 'PLAYER 3');

    if (!stats1 || !stats2 || !stats3) {
      console.log('\n❌ Could not analyze all players');
      return;
    }
    ...
    console.log(`  Player 3: ${p3.clerkUserId || 'No Clerk account'}`);
    ...
    console.log(`  Player 3: Created ${p3.createdAt.toISOString().split('T')[0]} (${Math.floor((Date.now() - p3.createdAt.getTime()) / (1000 * 60 * 60 * 24))} days ago)`);
    ...
    console.log(`    - Roster entries: ${stats3.rosterCount}`);
    console.log(`    - Lineup entries: ${stats3.lineupCount}`);
    console.log(`    - Registrations: ${stats3.registrations.length}`);
    console.log(`    - Paid registrations: ${stats3.registrations.filter(r => r.paymentStatus === 'PAID').length}`);
    console.log(`    - Captain roles: ${stats3.captainRoles}`);
    console.log(`    - Admin roles: ${stats3.adminRoles}`);
