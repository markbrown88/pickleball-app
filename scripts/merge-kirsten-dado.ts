...
async function mergeKirstenDado() {
  try {
    const keepId = 'cmieohsbh0003l204cv8nhry5'; // Newer account with Clerk, email, registration
    const deleteId = 'cmfpbp84m0029rdn0bby7831w'; // Older account with DUPR rating, location data

    console.log(`\n=== Merging Kirsten Dado Accounts ===\n`);
    console.log(`Keep: ${keepId} (newer, has Clerk account, email, registration)`);
    console.log(`Delete: ${deleteId} (older, has DUPR rating, location data)\n`);
    ...
    console.log(`\n✅ Successfully merged Kirsten Dado accounts!\n`);
    console.log(`Kirsten Dado (${keepPlayer.email}) now has:`);
